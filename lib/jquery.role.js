!function($){
  ...
  function rewriteSelector(context, name, argPos){
    ...
    context[name] = function(){
      arguments[argPos] = arguments[argPos].replace(
        /@([\w\u00c0-\uFFFF\-]+)/g,
        '[' + attributeName() + '~="$1"]'
      );
  ...
  $.extend($.fn, {
    roles: function(){ return parse(this.attr(attributeName())); },
    ...
    addRole: function(roleName){
      ...
      return this.each(function(_, element){
        var $el = $(element);
        $el.attr(attributeName(), parse($el.attr(attributeName()) + ' ' + roleName).join(' '));
      });
    },
    ...
    removeRole: function(roleName){
      ...
      return this.each(function(_, element){
        var $el = $(element);
        $el.attr(attributeName(), parse($el.attr(attributeName()), parse(roleName)).join(' '));
      });
    },
